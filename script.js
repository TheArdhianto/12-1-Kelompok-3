...
function openSidebar(poi) {
    ...
    function updateMarkerVisibility(selectedButton) {
        ...
        // 1. ITERASI SEMUA LAYER: Sembunyikan semua Layer Group
        ...
        // 3. AKTIFKAN YANG DIPILIH: Tambahkan layer dan status 'active' hanya pada yang baru diklik
        if (selectedButton) {
            ...
            if (!map.hasLayer(layerToDisplay)) {
                layerToDisplay.addTo(map);
            }
            
            // Beri status aktif pada tombol
    ...
    function filterContent(searchTerm) {
        ...
        mapMarkers.forEach(item => {
            ...
            // Cek apakah POI cocok dengan pencarian DAN cocok dengan kategori yang aktif
            const matchesSearch = poiName.includes(term) || poiDesc.includes(term);
            const matchesCategory = activeCategory === markerCategory; // Hanya tampilkan jika kategori marker aktif
    ...
    // PANGGILAN AWAL
    createMarkers(); 
    // Panggil visibilitas awal untuk semua tombol aktif
    categoryButtons.forEach(button => {
        updateMarkerVisibility(button);
    });
